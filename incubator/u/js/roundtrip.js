var _und = "undefined", __adroll = window.__adroll || {exp: 5E4, eexp: 720, pv: 1E11 * Math.random(), __adc: "__ar_v4", _nad: 0, _lce: null, _broken: !1, _url: 2E3, _r: {}, _kwl: 50, _logs: [], _secure: function () {
    ...
    var a = "" + b.getUTCFullYear(), c = b.getUTCMonth(), c = 10 <= c ? c : "0" + c, b = b.getUTCDate();
    ...
    check_cookie: function (a, b) {
        ...
        -1 != b.indexOf(a) ? this.set(this.__adc, this.check_cookie(b, a), this.exp) : this.set(this.__adc, [b, [a, this.get_date(this.eexp), "1"].join(":")].join("|"), this.exp)
    }, expire_old: function () {
        ...
        if (typeof this._r[b] == _und || d) {
            ...
            c ? (this.log("Rendering test ad " + c + " in space " + b), e.push("test_ad=" + c), a = this.get_url(a, b, null, "ad", e)) : d ? (this.log("Rendering campaign " + d), c = this.format_macros(!1, !1, !1, !1),
                e.push(c), a = this.get_url(a, b, d, null, e)) : (this.log("Rendering ad space " + b), a = this.get_url(a, b, null, "ad", e));
            this.expire_old();
        ...
        var d = typeof d == _und ? !1 : d, e = c ? escape : function (a) {
            return a
        }, f = {r: /^\$\{.*\}$/i, g: /^%%.*%%$/i, b: /^\[.*\]$/i, x: /^\$\{.*\}$/i, t: /INSERTCLICKTRACKER/}[adroll_ext_network], f = typeof f == _und ? /CANNOT_MATCH_THIS/ : f, h = adroll_cpm_macro, i = adroll_url_macro, k = b ? adroll_c_macro : null, g = [], j = a ? this.parseUri(a) :
            null, j = j ? this.endswith(j.path, ".tp") : !1;
        !j && d && g.push(["desturl", ""]);
        k && !f.test(k) && 0 === k.indexOf("http") ? (d = e, "g" == adroll_ext_network && (d = c ? function (a) {
            return a
        } : unescape), g.push(["clickurl", d(k)])) : j && d && g.push(["clickurl", ""]);
        adroll_ext_network && g.push(["adroll_network", adroll_ext_network]);
        h && !f.test(h) && g.push(["cpm", h]);
        adroll_subnetwork && !f.test(adroll_subnetwork) && g.push(["adroll_subnetwork", adroll_subnetwork]);
        adroll_ad_payload && !f.test(adroll_ad_payload) && g.push(["adroll_ad_payload", adroll_ad_payload]);
        i && !f.test(i) && (c = this.parseUri(unescape(i)), g.push(["site_url", e("http://" + c.host)]), b && (g.push(["adroll_width", e(adroll_width)]), g.push(["adroll_height", e(adroll_height)])));
        this.log("Macros found " + this.serialize(g));
        return a ? this.buildurl(a, g) : this.serialize(g)
    }, view: function (a) {
        var b = new Image;
        ...
        this.expire_old();
        var c = this._srv("/pixel"), d = document.createElement("script");
        ...
        this.addLoadEvent(function () {
            ...
            try {
                typeof adroll_segments !=
                    _und && f.push("name=" + escape(adroll_segments.toLowerCase()))
            } catch (i) {
            }
            try {
        ...
        var b = ["adroll_conversion_value", "adroll_conversion_value_in_dollars", "adroll_segments"], c, d, e = window, a = a || {};
        for (c = 0; c < b.length; c++)delete e[b[c]], b[c]in a && (e[b[c]] = a[b[c]], delete a[b[c]]);
        ...
        return typeof adroll_conversion_value != _und && !isNaN(parseInt(adroll_conversion_value)) ? "" + adroll_conversion_value : typeof adroll_conversion_value_in_dollars != _und && !isNaN(parseFloat(adroll_conversion_value_in_dollars)) ?
            "" + parseInt(100 * adroll_conversion_value_in_dollars) : null
    }, get_external_data: function () {
        if (typeof adroll_custom_data != _und) {
            var a = adroll_custom_data, b = {}, c;
        ...
        var f = null, h = this._gurl(), i = this.get_external_data();
        i && i.product_id && (f = i.product_id);
        if (!f && c && !("string" ==
            c && c instanceof String) && "html" == c.scheme) {
            if (e(b) || !0 != e(a))return"";
        ...
        var b = null, c;
        a.path && (window.jQuery ? (c = jQuery(a.path), c.length && (c = c.eq(0), b = "text" == a.attribute ?
            c.text() : c.attr(a.attribute))) : window.Prototype && window.$$ ? (c = $$(a.path), c.length && (c = c[0], b = "text" == a.attribute ? c.innerText && !window.opera ? c.innerText : c.innerHTML.stripScripts().unescapeHTML().replace(/[\n\r\s]+/g, " ") : c.readAttribute(a.attribute))) : window.YUI ? (c = YUI().use("node"), c.one && (c = c.one(a.path), b = null, c && (b = c.get(a.attribute).replace(/[\n\r\s]+/g, " ")))) : window.$$ && (c = $$(a.path), c.length && (c = c[0], b = "text" == a.attribute ? c.get("text") : c.getProperty(a.attribute))));
        if (b && (b = b.replace(/^\s\s*/,
            "").replace(/\s\s*$/, ""), a.regular_expression && a.regular_expression_replace && (c = RegExp(a.regular_expression, "gi"), c = c.exec(b), null != c))) {
        ...
        var b = [], c = this.get_external_data();
        if (!c)return null;
        for (var d in c)c.hasOwnProperty(d) &&
        b.push(this.normalize_var(escape("" + d) + "=" + escape("" + c[d])));
        b = b.join("&");
        a && (b = escape(b));
